...
To do so, our team ran performance testing using a machine equipped with a \emph{CPU type goes here} %TODO what was our CPU?
...
In future sections, whereever a graph appears with a label along the lines of ``Speedup over serial,'' (see Figure~\ref{lst:ContractFieldFieldScalar speedup over serial} as an example),
...
\begin{figure}
    \centering
    \subfloat[][Here is a graph showing the special case of Team Reduction algorithm's speedup over serial for \texttt{ContractFieldFieldScalar}. The faster speeds for the smaller contraction size (far left) is where the special case is used.]{\includegraphics[scale = .32]{ReductionSpecial2.png}
	\label{fig:CFFSSpecialReduction}}
	\:
	\subfloat[][\texttt{ContractFieldFieldScalar} team reduction's speedup over serial without the special case in the reduction. Notice that the smaller contraction sizes continue to perform worse, which was not the case for the reduction algorithm with the special case.]{\includegraphics[scale = .32]{ReductionNoSpecial2.PNG}
...
This shows that the adaptive slicing approach does have some process in these applications. These results were generated by loading two slices simultaneously in \texttt{ContractFieldFieldTensor} with $\ell = \mathcal{R} = 125$, $d_1 = d_2 = 4$, and $p$ ranging from $160$ to $320$.
