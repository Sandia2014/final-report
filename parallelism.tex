...
The question now is: how many threads per contraction is optimal? Well, looking
...
Now looking at Kokkos::Cuda which is using a LayoutLeft View, the memory needs
...
ContractFieldFieldScalar with $\ell = \mathcal{R} = 10, P = 8 - 1024$. We see
that in the corner where the memory size is small and contraction size is small
we get a small amount of speedup relative to independent Cuda code, which is
promising. This is the corner where we would expect slicing to perform the best
in comparison to independent parallelism, since in this corner flat parallelism
...
We're still working on an adaptive slicing implementation that can increase the
size of a block and load more data into shared memory when necessary. That
approach should benchmarked and included in the report by the end of this week! 
...
code assumes that tileSize (the horizontal and vertical dimensions of a tile)
...
In general, we have found that 2D tiling is the most effective method for
